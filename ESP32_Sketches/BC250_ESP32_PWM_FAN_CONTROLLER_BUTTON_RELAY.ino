...
// --------------------
// POWER RELAY / BISTABLE POWER BUTTON


const int RELAY_PIN = 4;    // drives the relay - stays HIGH / LOW
const int BUTTON_PIN = 16;  // physical button, wired to GND via resistor, uses internal pull-up (active LOW)

const unsigned long BUTTON_DEBOUNCE_MS = 80;

bool relayState = false; // true = relay closed / power on

struct PowerLog {
    time_t timestamp = 0;
    String source = "boot";  // "boot", "button", "ui", "api"
    String ip = "";
};
PowerLog lastPowerChange;
...
void setPower(bool state, String source, IPAddress ip) {

    relayState = state;
    digitalWrite(RELAY_PIN, relayState ? HIGH : LOW);

    prefs.putBool("relayState", relayState);

    lastPowerChange.timestamp = time(nullptr);
    lastPowerChange.source = source;
    lastPowerChange.ip = ip.toString();
}

void togglePower(String source, IPAddress ip) {
    setPower(!relayState, source, ip);
}

// debounced physical button - toggles power on each press (falling edge, active LOW)
void pollPowerButton() {
    ...
    if ((millis() - lastButtonDebounceTime) > BUTTON_DEBOUNCE_MS) {
        if (reading != buttonState) {
            ...
            if (buttonState == LOW) {
                togglePower("button", IPAddress(0, 0, 0, 0));
            }
        }
    }
...
<div class="power-bar">
    <div class="power-left">
        <strong>Power button state:</strong>
        <span class="power-pill off" id="powerPill">OFF</span>
    </div>
    <div style="display:flex; align-items:center; gap:12px;">
        <span class="power-meta" id="powerMeta">No changes since boot.</span>
...
function renderStatus(data) {
    ...
    pill.innerText = power.state.toUpperCase();
    pill.className = "power-pill " + (power.state === "on" ? "on" : "off");

    const meta = document.getElementById("powerMeta");
    if (power.time === "N/A") {
        meta.innerText = "No changes since boot.";
    } else {
        meta.innerText = `Last changed ${power.time} via ${power.source}`;
    }
}
...
// from the web UI - no key required, same trust level as /fan, /all, /setMode
void handlePowerUI() {
...
// from the server-side script - requires the shared API key
void handleApiPower() {
    ...
    String state = server.arg("state"); // "on" / "off" / "toggle"
    IPAddress ip = server.client().remoteIP();

    if (state == "on") {
        setPower(true, "api", ip);
    } else if (state == "off") {
        setPower(false, "api", ip);
...
// Status (fans + temperature + mode + last API call + power)
...
void handleStatus() {
    ...
    json += "\"time\":\"" + formatTime(lastPowerChange.timestamp) + "\",";
    json += "\"source\":\"" + lastPowerChange.source + "\",";
    json += "\"ip\":\"" + lastPowerChange.ip + "\"";
    json += "}";
...
void setup() {
    ...
    // Power relay: restore last known state from flash so an ESP reboot
    // doesn't cut power to a PC that's currently running.
    prefs.begin("fanctrl", false);
    relayState = prefs.getBool("relayState", false);
